...
public class Chessboard extends View implements IChessboard{
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
    	//为了适应不同手机配置文件或者这里w 和 h写死。本人选择在代码文件中写死
	...
	public void moduleChoose(int i){
		 if (currentMode == READY) {
	        	if(i==1){//向右键，人机对战
	        		player2 = computer;
	        	}else if(i==2){//向左键，人--人对战
	        		player2 = human;
	        	}
	        	restart();
	        	setMode(RUNNING);
	        }else if(currentMode==RUNNING &&i==-1){//重新开始
	        	restart();
	        	setMode(READY);
	        }else{
	        }
