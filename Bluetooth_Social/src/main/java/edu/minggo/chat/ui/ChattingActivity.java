...
import edu.minggo.chat.database.DatabaseHelper;
import edu.minggo.chat.database.MyContentProvider;
import edu.minggo.chat.database.MyProviderMetaData;
...
import static edu.minggo.chat.control.BluetoothChatService.context;
...
public class ChattingActivity extends MapActivity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		dh = MyContentProvider.dh;
	}

	private void initMessage(){
		Cursor cursor = dh.getReadableDatabase().query(MessageEntity.MessageTableMetaData.TABLE_NAME, null,
				MessageEntity.MessageTableMetaData.REMOTEDEVICENO + "= ?", new String[]{requestDeviceAdd}
				, null, null, null);
		while (cursor.moveToNext()){
			MessageEntity d;
			String readMessage = cursor.getString(cursor.getColumnIndex(MessageEntity.MessageTableMetaData.TTMCONTENT));
			int type = cursor.getInt(cursor.getColumnIndex(MessageEntity.MessageTableMetaData.type));
			int ttmType = cursor.getInt(cursor.getColumnIndex(MessageEntity.MessageTableMetaData.TTMTYPE));
			String date = cursor.getString(cursor.getColumnIndex(MessageEntity.MessageTableMetaData.DATE));
			if (ttmType == 1){
				if (type == 1){
					d = new MessageEntity(readMessage,R.layout.chatting_item_msg_text_right,"1",DateUtil.getCurrentTiem());
					d.setLayoutID(R.layout.chatting_item_msg_text_right);
				}
				else {
					d = new MessageEntity(readMessage,R.layout.chatting_item_msg_text_left,"0",DateUtil.getCurrentTiem());
	...
	public void onClick(View v) {
		...
		if (v==backbt) {
			...
			if(TextUtils.isEmpty(contentet.getText())){
				...
				values.put(MessageEntity.MessageTableMetaData.TTMCONTENT, sendContent);
				d.setLayoutID(R.layout.chatting_item_msg_text_right);
				values.put(MessageEntity.MessageTableMetaData.type, 1);
				d.setTtmType(1);
				values.put(MessageEntity.MessageTableMetaData.TTMTYPE, 1);
				d.setRemoteDeviceNo(requestDeviceAdd);
				values.put(MessageEntity.MessageTableMetaData.REMOTEDEVICENO, requestDeviceAdd);
				d.setMyDeviceNo(BluetoothChatService.mAdapter.getAddress());
				d.setTtmTime(DateUtil.getCurrentTiem());
				values.put(MessageEntity.MessageTableMetaData.DATE, DateUtil.getCurrentTiem());
				dh.getWritableDatabase().insert(MessageEntity.MessageTableMetaData.TABLE_NAME, null, values);
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_READ:
				...
				if (msg.arg1==0) {//收字符串
					...
					d.setMyDeviceNo(BluetoothChatService.mAdapter.getAddress());
					ContentValues values = new ContentValues();
					values.put(MessageEntity.MessageTableMetaData.TTMCONTENT, readMessage);
					values.put(MessageEntity.MessageTableMetaData.type, 0);
					values.put(MessageEntity.MessageTableMetaData.TTMTYPE, 1);
					values.put(MessageEntity.MessageTableMetaData.REMOTEDEVICENO, requestDeviceAdd);
					values.put(MessageEntity.MessageTableMetaData.DATE, DateUtil.getCurrentTiem());
					dh.getWritableDatabase().insert(MessageEntity.MessageTableMetaData.TABLE_NAME, null, values);
